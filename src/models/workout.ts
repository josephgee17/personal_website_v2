...
export const Workout = createModel<RootModel>()({
  ...
  effects: (dispatch) => ({
    async initSessionNumber(payload, rootState): Promise<void> {
      ...
      while (cache) {
        dispatch.Workout.setSessionVersion(rootState.Workout.sessionVersion + 1)
        cache = await localforage.getItem(
          testWorkoutDataID
        )
      }
    },
    ...
    async createWorkoutDaySession(payload, rootState): Promise<void> {
      ...
      const workoutDaySession: any = await localforage.getItem(
        workoutDataID
      )
